// SPDX-License-Identifier: GPL-3.0-or-later Sort Markdown Tables - A CLI tool for
// sorting markdown tables Copyright (C) 2025 Contributors
//
...
///
/// **`stdin_is_tty`** is injected so integration-style coverage of the help
/// shortcut does not require a real PTY (`should_print_help_when_stdin_tty`).
...
///
/// 3. For each file: a. Read contents b. Parse markdown c. Sort tables d. Collect
///    results (document + target)
///
/// 4. If any error during parsing/sorting: abort, report error, exit 2
///
/// 5. If --check mode: a. For each result, check if any tables were modified b. Report
...
mod tests {
    ...
    fn test_process_file_single_table() {
        ...
        // Bob before Alice, so unsorted
        assert!(process_result.check_results.iter().any(|r| !r.is_sorted));
    }
    ...
    fn test_process_file_already_sorted() {
        ...
        // Already in order
        assert!(process_result.check_results.iter().all(|r| r.is_sorted));
    }
    ...
    fn test_process_file_no_tables() {
        ...
        // No marked tables -> nothing to check
        assert!(process_result.check_results.is_empty());
    }
